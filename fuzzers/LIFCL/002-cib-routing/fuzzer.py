...
configs = [
    ((1, 18), FuzzConfig(job="CIBTROUTE", device="LIFCL-40", tiles=["CIB_R1C18:CIB_T"]), set(["TAP_CIBT_R1C14:TAP_CIBT"])),
    ((18, 1), FuzzConfig(job="CIBLRROUTE", device="LIFCL-40", tiles=["CIB_R18C1:CIB_LR"]), set(["TAP_PLC_R18C14:TAP_PLC"])),
    ((28, 17), FuzzConfig(job="CIBROUTE", device="LIFCL-40", tiles=["CIB_R28C17:CIB"]), set(["TAP_CIB_R28C14:TAP_CIB"])),
    ((28, 1), FuzzConfig(job="CIBLRAROUTE", device="LIFCL-40", tiles=["CIB_R28C1:CIB_LR_A"]), set(["TAP_CIB_R28C14:TAP_CIB"])),
]

async def per_config(rc, cfg, ignore, executor):
    cfg.setup()
    r, c = rc
    ...
    futures = fuzz_interconnect(config=cfg, nodenames=nodes, regex=True, bidir=True, ignore_tiles=ignore,
        pip_predicate=pip_filter, fc_filter=fc_filter, executor=executor) + \
...
async def run(executor):
    await asyncio.gather(*[(per_config(*cfg, executor=executor))for cfg in configs])
