...
async def main(executor):
    for feed, rc, ndcc, side, cfg in configs:
        ...
        fuzz_interconnect(config=cfg, nodenames=nodes, regex=False, bidir=False, full_mux_style=False,executor=executor)
        fuzz_interconnect(config=cfg, nodenames=mux_nodes, regex=False, bidir=False, full_mux_style=True,executor=executor)
        ...
        fuzz_interconnect(config=cfg, nodenames=["R{}C{}_J*".format(r, c)], regex=True, bidir=False, full_mux_style=False,
            pip_predicate=pip_filter, executor=executor)
